...
y){c.ui.position[l[x]]&&c.ui.position[l[x]][y](s,{targetWidth:j,targetHeight:n,elemWidth:p,elemHeight:r,collisionPosition:w,collisionWidth:u,collisionHeight:v,offset:k,my:g.my,at:g.at})});c.fn.bgiframe&&m.bgiframe();m.offset(c.extend(s,{using:g.using}))})};c.ui.position={fit:{left:function(g,h){var i=c(window);i=h.collisionPosition.left+h.collisionWidth-i.width()-i.scrollLeft();g.left=i>0?g.left-i:Math.max(g.left-h.collisionPosition.left,g.left)},top:function(g,h){var i=c(window);i=h.collisionPosition.top+
